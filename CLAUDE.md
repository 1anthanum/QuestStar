...
- **~60 components (54 top-level + 6 reflections), 25 hooks, 19 utils, 2 lib modules**
...
ios/                            # QuickTrack native iOS + macOS project (see ios/CLAUDE.md)
...
└── QuickTrack/                 # Xcode project root (generated by XcodeGen)
    ├── project.yml             # XcodeGen source of truth
    ├── QuickTrack/             # Host app target (SwiftUI)
    │   ├── Models/             # Tracker, TrackerSummary
    │   ├── DataSources/        # SyncManager, RetryQueue, SupabaseClient, adapters
    │   ├── Engine/             # RewardChain, CelebrationQueue, EngagementEngine, HapticEngine, ThemeManager, VisualEffects
    │   ├── Intents/            # LogEventIntent, CompleteStepIntent, RefreshSummaryIntent
    │   └── Views/              # TodayView, QuestsView, AchievementsView, SettingsView, etc.
    │       └── Today/          # Extracted TodayView sub-views (Water, Habits, OverlayStack)
    ├── QuickTrackWidget/       # Widget extension target (WidgetKit)
    │   ├── Widgets/            # Medication, QuestStar, Sleep, Water, DailyProgress widgets
    │   └── WidgetViews/        # Per-widget SwiftUI views (small/medium/large)
    └── Shared/                 # App Group shared code (AppGroupManager, Config, ColorHex)
```
...
<!-- Removed 2026-05-19: Quest Narrative, Seasonal Lore, and Boss Rush were documented as persisted but are actually stateless/computed-on-demand:
- narrativeEngine.js generates story frames from quest category at render time (no persistence)
- Seasonal events derive from current month, no per-month storage needed
- BossRush is a panel view that aggregates overdue quests; no persistent boss state -->
...
Native iOS + macOS app + widget project. Lives in `ios/` subdirectory of this repo. See `ios/CLAUDE.md` for full documentation.

**Status**: Phases 1–5 complete (host app, read-only widgets, write-back, engagement engine, reliability). Phase 6 (advanced features) ongoing.

**Key facts**:
- Reads/writes existing Supabase tables via PostgREST — zero changes to QuestStar web code
- 5 widgets: medication, QuestStar progress, sleep, water, daily progress
- Full SwiftUI host app with TodayView dashboard, quest management, achievements
- Engagement engine: dopamine reward chains (XP/coin/lore/level/quest), combos, personal bests, daily login bonus, weekly reports
- Swift 5.9 + SwiftUI + WidgetKit + App Intents + HealthKit
...
- Optimistic UI with RetryQueue + lifecycle-aware polling for battery efficiency
...
15. **Hook exports** — All hooks use **named** exports (`export function useX`). (An earlier note here claimed `useBlossomMode`/`useAccountabilityPact` used `export default`; that is no longer true — they are named exports like the rest.) JSX-containing hooks use the `.jsx` extension (`useAuth.jsx`, `useLanguage.jsx`); pure-logic hooks use `.js`.

16. **"Today" must be LOCAL date, everywhere** — `getTodayStr()` (and `todayKey()` in `layerEngine`) returns the **local** calendar date as `YYYY-MM-DD`, matching iOS's `Config.todayString()` which uses `DateFormatter` with no explicit timezone. **Never** use `new Date().toISOString().split("T")[0]` for "today" — that's the UTC date, and on any user past their UTC-offset cutoff (e.g., a Pacific user after ~17:00 PDT) it diverges from iOS's local key by a full day. R6-C1 was exactly this: every web "completed today" surface (rhythm ring, do-now, autoDefer, smart launcher, anti-paralysis prompt) read `habit_log[today]` where `today` was a different key than the one iOS had written under, so they all showed "nothing done" while iOS-marked completions sat in the previous-day bucket. Symptoms read like source-filtering; the cause was the date key. The same rule applies in tests: `tests/characterization/` use a `localKey()` helper for seeded log dates, not `toISOString()`. When adding any new "today" reader or writer, always go through `getTodayStr()` / `todayKey()`.
